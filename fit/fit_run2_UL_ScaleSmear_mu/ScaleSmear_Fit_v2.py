...
for a in a_masses:
    ...
    for year in years:
        ...
        file_param = open("./fit_param/paramDump_ggh_cat0_M"+str(a)+"_"+year+".txt",'r')
        file_param_root = TFile("./fit_param/paramDump_ggh_cat0_M"+str(a)+"_"+year+".root")
        ...
        for line in file_param.readlines()[1:-3]:
            ...
            if param_name in frac:
                param[a][year][param_name] = file_param_root.wsig_13TeV.func(frac[param_name]).getVal()
...
def sumGaussians(mass, param, scale):
    ...
    func = RooAddPdf("nSum", "nSum", RooArgList(gauss3,gauss2,gauss1,gauss0), RooArgList(coeff2,coeff1,coeff0))
...
def main():
    ...
    path_file = '/publicfs/cms/user/wangzebing/ALP/Analysis_code/fit/fit_run2_UL_ScaleSmear_mu/'
    path_out = '/publicfs/cms/user/wangzebing/ALP/Analysis_code/fit/fit_run2_UL_ScaleSmear_mu/ScaleSmear_plot/'
    ...
    for a_mass in a_masses:
        for year in years:
            ...
            for scsm in param_float:
                for p in sysType:
                    line = p + '_' + scsm + '_M' + str(a_mass)+'_'+year +'\t'
