...
sys_names = ['CMS_eff_g_up','CMS_eff_g_dn','CMS_pileup_up','CMS_pileup_dn','CMS_eff_lep_up','CMS_eff_lep_dn','norm']
...
for year in ['16', '16APV','17','18']:
    ...
    dataset_sys = {}
    for s in sys_names:
        dataset_sys[s] = RooDataSet("ggh_{0}_13TeV_cat0_{1}".format(mass_H,s),"ggh_{0}_13TeV_cat0_{0}".format(mass_H,s), ArgSet, "CMS_hza_weight")
    ...
    for jentry in range(entries):
        ...
        for s in sys_names:
            if s =='CMS_eff_g_up':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeight * mychain.l1_dataMC * mychain.l2_dataMC * mychain.event_weight * (mychain.pho1SFs+mychain.pho1SFs_sys) * (mychain.pho2SFs+mychain.pho2SFs_sys)
            elif s =='CMS_eff_g_dn':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeight * mychain.l1_dataMC * mychain.l2_dataMC * mychain.event_weight * (mychain.pho1SFs-mychain.pho1SFs_sys) * (mychain.pho2SFs-mychain.pho2SFs_sys)
            elif s =='CMS_pileup_up':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeightUp * mychain.l1_dataMC * mychain.l2_dataMC * mychain.event_weight * mychain.pho1SFs * mychain.pho2SFs
            elif s =='CMS_pileup_dn':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeightDn * mychain.l1_dataMC * mychain.l2_dataMC * mychain.event_weight * mychain.pho1SFs * mychain.pho2SFs
            elif s =='CMS_eff_lep_up':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeight * (mychain.l1_dataMC+mychain.l1_dataMCErr) * (mychain.l2_dataMC+mychain.l2_dataMCErr) * mychain.event_weight * mychain.pho1SFs * mychain.pho2SFs
            elif s =='CMS_eff_lep_dn':
                weight_sys = mychain.event_genWeight * mychain.event_pileupWeight * (mychain.l1_dataMC-mychain.l1_dataMCErr) * (mychain.l2_dataMC-mychain.l2_dataMCErr) * mychain.event_weight * mychain.pho1SFs * mychain.pho2SFs
            elif s =='norm':
                weight_sys = 1.0

            dataset_sys[s].add(ArgSet,weight_sys)
    ...
    for s in sys_names:
        getattr(w,'import')(dataset_sys[s])
    #getattr(w,'import')(dataset_WithoutWeight)
    if args.ele:
        w.writeToFile("ALP_data_sig_Am{0}_Hm{1}_{2}_workspace_ele.root".format(mass,mass_H,year))
    elif args.mu:
        w.writeToFile("ALP_data_sig_Am{0}_Hm{1}_{2}_workspace_mu.root".format(mass,mass_H,year))
    else:
