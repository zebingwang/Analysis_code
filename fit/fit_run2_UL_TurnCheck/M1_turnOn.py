...
histos_H= TH1F('H_m', 'H_m', 50,  95., 180.)
...
for jentry in range(entries):
    ...
    histos_H.Fill(mychain.H_m,mychain.factor*mychain.pho1SFs*mychain.pho2SFs)
    
    

c = TCanvas("c","c")
c.cd()
histos_H.GetYaxis().SetTitle('Events / (%.2f GeV)' %histos_H.GetBinWidth(1))
histos_H.GetXaxis().SetTitle('m(h) GeV')
histos_H.Draw("HIST")
#c.SaveAs("mass1_dataCR_neu"+str(mvaCut)+".png".format(mass))
c.SaveAs("mass{0}_dataCR_neu{1}.png".format(mass,mvaCut))
