...
entries_data = mychain_data.GetEntries()
print entries_data
...
entries_DY = mychain_DY.GetEntries()

histos_H_data= TH1F('H_m_data', 'H_m_data', 17,  95., 180.)
histos_H_DY= TH1F('H_m_DY', 'H_m_DY', 17,  95., 180.)
'''
for jentry in range(entries_data):
    ...
    histos_H_data.Fill(mychain_data.H_m)
'''
...
c = TCanvas("c","c")
c.cd()
histos_H_DY.SetFillColor(kBlue)
histos_H_DY.SetMarkerStyle(20)
histos_H_DY.GetYaxis().SetTitle('Events / (%.2f GeV)' %histos_H_data.GetBinWidth(1))
histos_H_DY.GetXaxis().SetTitle('m_llgg (GeV)')
histos_H_DY.SetTitle("")

#legend = TLegend(0.7,0.8,0.88,0.88)
#legend.AddEntry(histos_H_data,"data")
#legend.AddEntry(histos_H_DY,"DY")

#histos_H_data.Draw("PE")
histos_H_DY.Draw("HISTSAME")
#legend.Draw("SAME")
#histos_H_data.Draw("SAMEPE")
#histos_H_data.Draw("Axissame")
