...
mvaCuts = {5:0.98, 15:0.98, 30:0.975}
mvaCut = mvaCuts[mass]
model = pickle.load(open(BDT_filename, 'rb'))

####################
filename = '/publicfs/cms/user/wangzebing/ALP/Analysis_out/18/massInde/ALP_M{0}.root'.format(mass)
...
for mass_H in [125 - shift, 125, 125 + shift]:
    ...
    IntLumi = RooRealVar("IntLumi","IntLumi",56.9)
    ...
    w.writeToFile("ALP_data_sig_Am{0}_Hm{1}_workspace.root".format(mass,mass_H))
    del w
